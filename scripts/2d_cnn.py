...
from src.models import create_2DCNN
...
if model_chkpt_path is None:
    model_chkpt_path = f"../checkpoints/2d_cnn_ckpt_test={train_test_split_id}_lenthresh={length_thresh}.pt"
...
num_classes = 3
metrics_df = pd.DataFrame(columns=["test_idx", "auroc", "aupr", *[f"f1_class{i}" for i in range(num_classes)]])
device = get_device()
model_id = "2d_cnn"
...
# Load balanced test data and preprocess
if to_eval:
...
signal_processor = SignalProcessor(
    pad=True, pad_value=2, pad_len=(rescale_dim * rescale_dim), preserve_longer_than_pad_length=True,
    add_channels_dim=True,
    reshape=True, rescale_dim=rescale_dim, stack_depth=stack_depth, shuffle=True
)
if to_train:
...
if to_sim:
    signal_processor.shuffle = False
...
model = create_2DCNN(input_data_length=input_data_length, num_classes=num_classes, in_channels=stack_depth)
model.to(device)
...
if to_train:
    ...
    plot_losses(train_losses, val_losses, model_description="2D Classifier")
    save_model(model, f"../checkpoints/{model_id}_ckpt_test={train_test_split_id}_lenthresh={length_thresh}_rescale={rescale_dim}_stack={stack_depth}.pt")
...
if to_eval:
    ...
    metrics_df.to_csv(f"../res/{model_id}_metrics_test={train_test_split_id}_lenthresh={length_thresh}_rescale={rescale_dim}_stack={stack_depth}.csv")
...
if to_sim:
    ...
    for (l, simloader) in enumerate(simloaders):
        ...
        # Plot the number of predictions for each class over time
        plot_simulation(end_times, class_pred_counts, class_pred_prob_avgs, pred_proba_all, labels_key, l, num_classes, filename=f"../res/{model_id}_simulation_test={train_test_split_id}_lenthresh={length_thresh}_class={l}.png")